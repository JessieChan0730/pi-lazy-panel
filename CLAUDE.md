...
│   ├── mouse.ts              # 纯函数：panelGeometry（面板列/行切分，render 和鼠标共用，保证画的位置=点的位置）+ hitTest（单元格 (x,y) → 面板 + 列表行号，SESSIONS 每行 2 行、TREE 每行 1 行，边框/空白/页脚返回无行）。仅 fullscreen TUI 模式下才收到鼠标事件，见 docs/issues.md
